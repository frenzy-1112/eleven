...
clone() {
    msg "|| Cloning toolchains and libufdt ||"
...
build_kernel() {
    ...
    if [ "$DEF_REG" = 1 ]; then
        ...
        git commit -m "$DEFCONFIG: Regenerate — auto-generated" || true
    fi
    ...
    make -j"$PROCS" O=out "${MAKE_ARGS[@]}" > build.log 2>&1 || err "❌ Kernel build failed!"
    ...
    if [ -f "$OUT_DIR/arch/arm64/boot/Image" ]; then
        ...
        if [ "$BUILD_DTBO" = 1 ]; then
            ...
            "$PYTHON" "$UFDT_DIR/utils/src/mkdtboimg.py" create "$DTBO_OUTPUT" --page_size=4096 "$DTBO_SOURCE"
        fi

        gen_zip
    else
        err "❌ Kernel build output not found"
    fi
}
...
gen_zip() {
    ...
    cd ..
    msg "✅ Flashable zip created: $AK3_DIR/$ZIPNAME-$DEVICE-$DATE.zip"
}
...
echo -e "$green Build completed in $((DIFF / 60)) min $((DIFF % 60)) sec.$default"
